...
| **Phase 2** | Cooperation multi-agent reelle | 🔄 EN COURS (§A-§AB ✅ gouvernance corrective, mode auto/debug, dispatch dry-run tracable) |
...
- **Contrats de sortie par task** (`crew/contracts.py`, Phase 1 §1) : chaque
  ...
  Le CLI peut charger ce ledger avec `--correction-ledger-json`, verifier `schema_version` quand il est present, ecrire un snapshot de reprise avec `--correction-ledger-out-json`, produire un manifeste dry-run de dispatch avec `--correction-dispatch-json`; son résumé CLI consomme directement les IDs top-level quand ils sont présents, et ecrire directement le prochain ledger reutilisable avec `--correction-next-ledger-json` en affichant aussi le résumé du dispatch projeté.
...
│   ├── test_modes.py        # Tests unitaires modes d'usage Phase 1 §2 + gardes JSON/ledger/dispatch + mode auto/debug (94/94)
