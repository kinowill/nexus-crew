...
| **Phase 2** | Cooperation multi-agent reelle | 🔄 EN COURS (§A-§AC ✅ gouvernance corrective, mode auto/debug, dispatch dry-run tracable) |
...
- **Contrats de sortie par task** (`crew/contracts.py`, Phase 1 §1) : chaque
  ...
  en cas de blocage. `--strict-correction-dispatch` retourne exit code 3
  quand un dispatch correctif dry-run est disponible, avec résumé CLI et IDs, sans retry automatique ni recalcul du payload quand il a déjà été produit.
  `--governance-json` ecrit un rapport machine-readable
  ...
  Le CLI peut charger ce ledger avec `--correction-ledger-json`, verifier `schema_version` quand il est present, ecrire un snapshot de reprise avec `--correction-ledger-out-json`, produire un manifeste dry-run de dispatch avec `--correction-dispatch-json`, incluant les enveloppes dispatchables et bloquées; son résumé CLI consomme directement les IDs top-level quand ils sont présents, et ecrire directement le prochain ledger reutilisable avec `--correction-next-ledger-json` en affichant aussi le résumé du dispatch projeté.
...
| `--strict-correction-dispatch` | Retourne exit code 3 si un dispatch correctif dry-run est disponible (pas de retry auto) |
...
| `--correction-dispatch-json` | Ecrit un manifeste dry-run des interactions dispatchables et du prochain ledger, puis imprime son résumé CLI avec IDs (pas de retry auto) |
...
│   ├── test_modes.py        # Tests unitaires modes d'usage Phase 1 §2 + gardes JSON/ledger/dispatch + mode auto/debug (97/97)
...
- Les violations de contrat produisent maintenant un etat de gouvernance
  ...
  `interaction_type`, `interaction_id`, `corrective_actions`, snapshot ledger versionne, manifeste dispatch dry-run avec résumé CLI et IDs au premier niveau JSON, next ledger relisible et exit strict optionnel, mais ne
