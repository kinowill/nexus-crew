...
| **Phase 2** | Cooperation multi-agent reelle | 🔄 EN COURS (§A-§N ✅ gouvernance corrective, JSON versionne, ledger snapshot) |
...
- **Contrats de sortie par task** (`crew/contracts.py`, Phase 1 §1) : chaque
  ...
  Le CLI peut charger ce ledger avec `--correction-ledger-json` et ecrire un snapshot de reprise avec `--correction-ledger-out-json`.
...
│   ├── test_modes.py        # Tests unitaires modes d'usage Phase 1 §2 + gardes JSON/ledger (44/44)
...
- Les violations de contrat produisent maintenant un etat de gouvernance
  ...
  `interaction_type`, `interaction_id`, `corrective_actions` et snapshot ledger, mais ne
