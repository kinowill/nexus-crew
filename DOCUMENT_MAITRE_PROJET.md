...
- **§AC — Enveloppes bloquées dans le manifeste dispatch** : ✅ FAIT (2026-07-17). Le manifeste dry-run expose maintenant `blocked_interactions` en plus de `blocked_interaction_ids`, symétriquement à `dispatchable_interactions`. Les consommateurs peuvent comprendre pourquoi une interaction est bloquée sans recalculer ni ouvrir un autre rapport; extension compatible, sans bump de schéma ni retry automatique.
...
- `scripts/test_modes.py` — tests unitaires modes d'usage Phase 1 §2 + gardes chemin JSON/ledger/dispatch, snapshot correctif, next ledger, statuts dispatch, résumés CLI avec IDs, mode auto/debug, exit strict et schémas (97/97)
...
### 2026-07-17 — Phase 2 §AC / Enveloppes bloquées dans le manifeste dispatch

- **Scope** : `crew/crew.py`, `scripts/test_modes.py`, `README.md`, `DOCUMENT_MAITRE_PROJET.md`.
- **Motivation** : le manifeste dispatch exposait les enveloppes complètes des interactions dispatchables, mais seulement les IDs des interactions bloquées. Un consommateur devait donc recalculer ou croiser avec un autre rapport pour comprendre les blocages.
- **Changement appliqué** : `_correction_dispatch_payload()` expose maintenant `blocked_interactions` avec les enveloppes complètes non dispatchables, en plus de `blocked_interaction_ids`. `_correction_dispatch_summary()` sait aussi retrouver les IDs bloqués depuis ces enveloppes si les IDs top-level ne sont pas présents. Aucun dispatch réel, aucun ledger consommé, aucun retry automatique.
- **Tests ajoutés** : `scripts/test_modes.py` vérifie le payload bloqué, le fallback du résumé depuis `blocked_interactions`, et le fichier écrit par `--correction-dispatch-json` dans un cas budget épuisé.
- **Validation offline** : `test_contracts.py` 76/76 OK, `test_modes.py` 97/97 OK, `test_resilience.py` 31/31 OK, `test_phase0.py` 22/22 OK, `crew.py --help` OK, `ruff check crew scripts` OK, `py_compile` OK, `git diff --check` OK.
- **Repo modifié** : oui.
- **Prod alignée** : N/A (outil local).
- **Validation runtime NIM** : non nécessaire pour cette slice manifeste JSON/tests offline uniquement.
- **Commit** : *(ce commit).*
...
- **Scope** : `crew/crew.py`, `scripts/test_modes.py`, `README.md`, `DOCUMENT_MAITRE_PROJET.md`.
- **Motivation** : après §Y, le manifeste expose `dispatchable_interaction_ids` au premier niveau, mais le résumé CLI recalculait encore les IDs depuis les enveloppes complètes.
- **Changement appliqué** : `_correction_dispatch_summary()` consomme désormais `dispatchable_interaction_ids` quand il existe, avec fallback compatible sur `dispatchable_interactions` pour les payloads anciens ou construits manuellement. Aucun changement de schéma, aucun dispatch réel, aucun retry automatique.
- **Tests ajoutés** : `scripts/test_modes.py` couvre le résumé sur payload IDs-only et le fallback via enveloppes dispatchables.
- **Validation offline** : `test_contracts.py` 76/76 OK, `test_modes.py` 91/91 OK, `test_resilience.py` 31/31 OK, `test_phase0.py` 22/22 OK, `crew.py --help` OK, `ruff check crew scripts` OK, `py_compile` OK, `git diff --check` OK.
- **Repo modifié** : oui.
- **Prod alignée** : N/A (outil local).
- **Validation runtime NIM** : non nécessaire pour cette slice de résumé CLI/tests offline uniquement.
- **Commit** : *(ce commit).*
